 ...
 *
 *	Modificado, no reemplazar por nuevas versiones porque no va a funcionar.
 *
...
/**
 * SetRequired sirve para cambiar dinamicamente el requerido
 * Para agregar validaciones customizadas agregar al elemento una funcion de la siguiente forma
 * 
  	$("#elementoAValidar").data('customValidate', function(){	
		if(--- es valido --- ){
			return "";
		} 
...
(function($) {
	$.fn.setRequired= function(required){
		if(required){
	...
	$.fn.setEnabled= function(enabled){
		...
		if(enabled){
			...
			input.css("color","");
			if(input.val()=="" && input.data("text")!="")input.val(input.data("text")); 
		}else{ 
			input[0].disabled=true;
			input.data("text",input.val());
			input.val("");  
			input.css("background-color","#F1F1F3");
			input.css("color","#F1F1F3");
		}
	};
	$.fn.setReadOnly= function(readOnly){ 
		$(this)[0].disabled=readOnly;
		if(readOnly){
...
//# REQUIRED|PATTERN|NUMBER|DATETIME|EMAILURL|MAXLENGTH
			...
			colorErr: 'white',
//# REQUIRED
			emptyMessage: 'Complete este campo.',
			unselectedMessage: 'Seleccione un item de la lista.',
//# PATTERN
			patternMessage: 'No concuerda con el patron solicitado.',
//# EMAILURL
			emailMessage: 'La direccion de email no es correcta.',
			urlMessage: 'La URL no es correcta.',
//# PLACEHOLDER
			colorOff: '#a1a1a1',
//# MAXLENGTH
			maxlengthMessage: 'Too many # characters.',
//# NUMBER|DATETIME
			invalidMessage: 'El valor es invalido.',
			minMessage: 'El valor tiene que ser mayor o igual a #.',
			maxMessage: 'El valor tiene que ser menor o igual a #.',
//# NUMBER
...
//#
		...
		var ua = window.navigator.userAgent.toLowerCase(),
			...
			isAndroid = parseInt(ua.replace(/.*android (\d+).*/, '$1'));
...
//#
				validatable = ':input:not(:button, :submit, :radio, :checkbox)',
				...
				form.data('customValidity',new Object());
				form.data('validatableElements',form.find(validatable));
//# REQUIRED|PATTERN|NUMBER|DATETIME|EMAILURL|MAXLENGTH
			/**
			...
			$.fn.setCustomValidity = function(customValidity, message) {
				...
				if (ui.is(validatable)) {
					...
					if (hasCustomValidity) {
						...
						var i =ui.parents("form").find(validatable).index(ui);
						ui.data('html',ui.outerHTML());
						if (message) {
							customValidity[i] = new Array(ui, message.replace(/\n/, '<br />'));
//							ui.css('border', '1px solid red');
							//ui.addClass('h5form-error');
						} else {
							customValidity[i] = new Array(ui, null);
							//ui.removeClass('h5form-error');
						}
					}
				}
				return ui;
...
//# NUMBER|DATETIME
			...
			$.fn.spin = function(isDown) {
				var ui = $(this),
					...
					base = (isNumber) ? min : 0,
					val = (isNumber) ? (isNaN(Number(ui.val()))? 0 : Number(ui.val())) : str2sec(ui.val(), true);
...
//#
			...
			$.fn.initControl = function() { 
				return this.each(function() {
					...
					var evNumberBlur = (function(){
						if(/^-?[0-9]*[\.,]{0,1}[0-9]*$/.test($(this).val())) {
							$(this).val($(this).val().replace(".",",")); 
						}				
					});
					if(type == 'positivenumber'){
						ui.attr("pattern","^[0-9]+([\.\,][0-9]+)?$");
						ui.attr("title","Complete un valor positivo");
						ui.bind('blur', evNumberBlur);
					}
					  
					if(type == 'customnumber'){
						ui.attr("pattern","^-?[0-9]+([\.\,][0-9]+)?$");
						ui.attr("title","Complete un valor ");
						ui.bind('blur', evNumberBlur);
					}
//# AUTOFOCUS
					// Is autofoucs
...
//# NUMBER|DATETIME
						...
						var evKeydown = (function(ev) {
							...
							if (cc == 38) ui.spin(0);
							if (cc == 40) ui.spin(1);
							if (($.inArray(cc, allow) >= 0) || (cc >= 48 && cc <= 57) || (cc >= 96 && cc <= 105)) return true;
							return false;
						});
...
//# DATETIME
					// Datepicker
					if ((type == 'customdate') && ('datepicker' in ui)) {
						var option = { dateFormat: 'dd/mm/yy' };
						option.minDate = ui.getAttr('min');
						option.maxDate = ui.getAttr('max');
						ui.datepicker({buttonImageOnly: true, showOn: 'button', buttonImage: '/images/calendar_icon.gif', 
							monthNames: ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'],
							monthNamesShort: ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'], 
							dayNamesMin: ['Do', 'Lu', 'Ma', 'Mi', 'Ju', 'Vi', 'Sa'],
							dayNames: ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'],
							nextText: 'Siguiente',
							minDate:ui.getAttr('min'), 
							maxDate:ui.getAttr('max'), 
							prevText: 'Anterior',  
							dateFormat: 'dd/mm/yy'});
...
//# REQUIRED|PATTERN|NUMBER|DATETIME|EMAILURL|MAXLENGTH
						...
						ui.setCustomValidity(form.data('customValidity'), null);
//#
						if (hasBugEnter && !ui.is('select, textarea, button')) {
							...
							var evKeypress2 = (function(ev) {
								...
								if (cc == 13 && form.find('input:submit, button:submit') && isAndroid) {
									form.find('input:submit, button:submit').eq(0).click();
								}else if (cc == 13 && form.find('input:submit, button:submit')) {
									form.find('input:submit, button:submit').eq(0).click();
									return false;
								}
								return true;
						...
						if(ui.data('customValidate')){
							var validateResult = ui.data('customValidate')();
							if(validateResult!= ''){
								ui.setCustomValidity(form.data('customValidity'), validateResult);
							}
						}
...
//# REQUIRED
						...
						if (!hasRequired && ui.getAttr('required')) {
							isNecessary = true;
							if (isEmpty) {
								ui.setCustomValidity(form.data('customValidity'), (ui.is('select')) ?
...
//# PATTERN
						...
						if (!hasPattern && (pattern = ui.getAttr('pattern'))) {
							...
							if (!isEmpty &&
								...
								ui.setCustomValidity(form.data('customValidity'), opts.patternMessage);
								return true;
							}
						}
//# EMAILURL
						...
						if (!hasEmail && type == 'email') {
							...
							if (!isEmpty && validateRe(ui,
							   '[\\w-\\.]{3,}@([\\w-]{2,}\\.)*([\\w-]{2,}\\.)[\\w-]{2,4}', 'i')) {
								ui.setCustomValidity(form.data('customValidity'), opts.emailMessage);
								return true;
							}
						}
						...
						if (!hasUrl && type == 'url') {
							...
							if (!isEmpty && validateRe(ui,
							   '[\\w-\\.]{3,}:\\/\\/([\\w-]{2,}\\.)*([\\w-]{2,}\\.)[\\w-]{2,4}',
							   'i')) {
								ui.setCustomValidity(form.data('customValidity'), opts.urlMessage);
								return true;
							}
						}
...
//# MAXLENGTH
						...
						if (!hasMaxlength && ui.is('textarea') && ui.getAttr('maxlength')) {
							...
							if (over = validateMaxlength(ui)) {
								ui.setCustomValidity(form.data('customValidity'),
...
//# DATETIME
							...
							if (ui.parent().hasClass(opts.classDatetime)) {
								...
								ui2.setCustomValidity(form.data('customValidity'), '');
...
//# NUMBER|DATETIME
							...
							if (validateRe(ui0, pattern) || (validateStep(ui0, min, step))) {
								ui2.setCustomValidity(form.data('customValidity'), opts.invalidMessage);
								return true;
							}
							if (validateMin(ui0)) {
								ui2.setCustomValidity(form.data('customValidity'),
...
//#
					...
					if(window.navigator.userAgent.indexOf("MSIE")!=-1){ 
						ui[0].onkeypress=evChange;  
					}
					
					//ui.bind('rebind', evChange);
					
 
				});

			};
			form.data('validatableElements').initControl();
...
//# REQUIRED|PATTERN|NUMBER|DATETIME|EMAILURL|MAXLENGTH
						if (attr == 'novalidate') {
							form.data('validatableElements').each(function() {
								$(this).setCustomValidity(form.data('customValidity'), '');
							});
						}
//# FORM
					}
...
//# REQUIRED|PATTERN|NUMBER|DATETIME|EMAILURL|MAXLENGTH
				...
				if (!hasCustomValidity) {
					var i;
					for (var j in form.data('customValidity')) {
						if (form.data('customValidity')[j][1] && (i == null || i > j) && !form.data('customValidity')[j][0][0].disabled ) i = j;
					}
					if (i != null) {
						var err = form.data('customValidity')[i][0],
							message = form.data('customValidity')[i][1];
						err = err.eq(0);
...
//#
			...
			$.fn.isValid = function(){
				if (!hasCustomValidity) {
					var i;
					for (var j in $(this).data('customValidity')) {
						if ($(this).data('customValidity')[j][1] && (i == null || i > j) && !$(this).data('customValidity')[j][0][0].disabled ) i = j;
					}
					return i == null;
				}else{
					return $(this)[0] && $(this)[0].checkValidity();
					
				}
			};

			$.fn.refresh= function(){
				$(this).data('customValidity', new Object());
				$(this).data('validatableElements',$(this).find(validatable));
				if($(this).data('validatableElements')){
					$(this).data('validatableElements').initControl();
				};
			};
			...
			$.fn.validate = function(){
				...
				if(!isValid){
					$(this).append('<button type="submit" id="toDelete" style="display:none"/>');
					if($('#toDelete')[0]) $('#toDelete')[0].click();
					$('#toDelete').remove();
				}
				return isValid;
			};
			...
			$.fn.extend({ serialize: function(){
				...
				var serialized= jQuery.param( $(this).serializeArray() );
				// Focus and blur attach for Placeholder
...
//# REQUIRED|PATTERN|NUMBER|DATETIME|EMAILURL|MAXLENGTH
				...
				if (!hasCustomValidity) {
					...
					if(!isValid){
						form.append('<button type="submit" id="toDelete" style="display:none"/>');
						$('#toDelete')[0].click();
						$('#toDelete').remove();
					}
					return isValid; 
